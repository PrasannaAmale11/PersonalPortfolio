...
const Data = () => {
  ...
  useEffect(() => {
    ...
    const letters = document.querySelectorAll(".home__title span");
    ...
    const moveCircle = (e) => {
      ...
      gsap.to(circle, {
        x: e.clientX - circleWidth / 2 + -10,
        y: e.clientY - circleHeight / 2,
        duration: 0,
      });

      // function to chack if letters are inside circle
      letters.forEach((letter) => {
        const letterRect = letter.getBoundingClientRect();
        const isLetterInsideCircle =
          letterRect.left < circleRect.right &&
          letterRect.right > circleRect.left &&
          letterRect.top < circleRect.bottom &&
          letterRect.bottom > circleRect.top;

        if (isLetterInsideCircle) {
          letter.style.color = "white";

          letter.style.transition = "all 0.3s ease ";
        } else {
          letter.style.color = "hsl(0, 0%, 46%)";
        }
      });
    };
    ...
    letters.forEach((letter) => {
      letter.addEventListener("mouseenter", () => {
        ...
        circle.style.background =
          "-webkit-linear-gradient(77deg, #000000 0%, #7f7f7f 100%)";
        circle.style.background =
          "linear-gradient(77deg, #000000 0%, #7f7f7f 100%)";
      });
  ...
  return (
    <div className="home__data">
      ...
      ></div>
      <h1 className="home__title" style={{ display: "flex" }}>
        {Array.from("Prasanna Amale").map((letter, index) => (
      ...
      <p className="home__description">
        I'm creative devloper based in Maharashtra, and I am very passionate and
        dedicated to my work
      </p>
      <a href="#contact" className="button button--flex sendMessageHome">
        Send Message
        <svg
